...
contract SupplierTrustScore {
    ...
    function setSupplierScore(address supplierAddress, uint256 trustableReviewsScore, uint256 totalCost, uint256 timelySupplyScore) external onlyOwner {
        ...
        require(totalCost >= 1 && totalCost <= 100, "Total cost must be between 1000 and 1200.");
        require(timelySupplyScore >= 1 && timelySupplyScore <= 5, "Timely supply score must be between 1 and 10.");
    ...
    function calculateFinalScore(address supplierAddress) public view returns (uint256) {
        ...
        uint256 trustableReviewsWeight = 4;  
        uint256 totalCostWeight = 3;         
        uint256 timelySupplyWeight = 3;   
        ...
        uint256 x = (weightedTrustableReviewsScore + weightedTotalCost + weightedTimelySupplyScore) / totalWeight;
        uint256 finalScore = (x/33)*100;
